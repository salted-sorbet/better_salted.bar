...
Item {
  ...
  // Demoted from `required`: plugin bars load without initial properties;
  // shell.configureBar assigns these right after the Loader finishes.
  ...
  // ---- Floating islands (ryan.bar) ----
  // bar.islands in shell.json: { enabled, edgeGap, sideGap, radius, padding,
  // widths: { left, center, right } }. Widths are extra pixels beyond the
  // content-fit size; drag an island's edges to change them. Islands render
  // per section with independent rounded backgrounds, so the bar reads as
  // three floating pills instead of one slab glued to the screen edge.
  ...
  property var islandWidths: ({})
  property var islandWidthLive: ({})
  property string islandDragRegion: ""
  property string islandDropRegion: ""
  property point islandDragStartPos: Qt.point(0, 0)
  property point islandDragPos: Qt.point(0, 0)
  readonly property bool islandsActive: islandsEnabled
  ...
  // ---- Island helpers ----
  function islandPositiveInt(value, fallback) {
    var n = Number(value)
    return isFinite(n) && n > 0 ? Math.round(n) : fallback
  }

  function islandExtraWidth(region) {
    var value = islandWidthLive[region]
    if (value === undefined || value === null) value = islandWidths[region]
    var n = Number(value)
    return isFinite(n) && n > 0 ? n : 0
  }

  function setIslandWidthLive(region, value) {
    var next = {}
    for (var key in islandWidthLive) next[key] = islandWidthLive[key]
    next[region] = Math.max(0, Math.round(value))
    islandWidthLive = next
  }

  function clearIslandWidthLive() {
    islandWidthLive = {}
  }
  ...
  function commitIslandWidth(region) {
    ...
    root.shell.mutateShellConfig(function(config) {
      if (!Util.isPlainObject(config.bar)) config.bar = {}
      var islands = Util.isPlainObject(config.bar.islands) ? config.bar.islands : (config.bar.islands = {})
      var widths = Util.isPlainObject(islands.widths) ? islands.widths : (islands.widths = {})
      if (extra > 0) widths[region] = extra
      else delete widths[region]
    })
  }

  function beginIslandMove(region, point) {
    // Without mutateShellConfig a swap could not be persisted, so keep the
    // gesture click-only there.
    if (!root.shell || typeof root.shell.mutateShellConfig !== "function") return
    islandDragRegion = region
    islandDropRegion = ""
    islandDragStartPos = point
    islandDragPos = point
  }

  function updateIslandMove(point, islandsMap) {
    if (!islandDragRegion) return
    islandDragPos = point
    var hit = ""
    var order = ["left", "center", "right"]
    for (var i = 0; i < order.length; i++) {
      var region = order[i]
      if (region === islandDragRegion) continue
      var item = islandsMap ? islandsMap[region] : null
      if (!item || !item.visible) continue
      try {
        var topLeft = item.mapToItem(null, 0, 0)
        if (point.x >= topLeft.x && point.x <= topLeft.x + item.width &&
            point.y >= topLeft.y && point.y <= topLeft.y + item.height) {
          hit = region
          break
        }
      } catch (e) {
    ...
    islandDropRegion = hit
  }

  function finishIslandMove() {
    var from = islandDragRegion
    var to = islandDropRegion
    islandDragRegion = ""
    islandDropRegion = ""
    if (from && to && from !== to) swapIslandRegions(from, to)
  }

  function cancelIslandMove() {
    islandDragRegion = ""
    islandDropRegion = ""
  }

  function swapIslandRegions(fromRegion, toRegion) {
    ...
    root.shell.mutateShellConfig(function(config) {
      ...
      var fromEntries = Array.isArray(config.bar.layout[fromRegion]) ? config.bar.layout[fromRegion] : []
      var toEntries = Array.isArray(config.bar.layout[toRegion]) ? config.bar.layout[toRegion] : []
      config.bar.layout[fromRegion] = toEntries
      config.bar.layout[toRegion] = fromEntries

      // Width extras belong to the slot the content lands in.
      if (!Util.isPlainObject(config.bar.islands)) config.bar.islands = {}
      var islands = config.bar.islands
      if (!Util.isPlainObject(islands.widths)) islands.widths = {}
      var widths = islands.widths
      var movedOut = widths[fromRegion]
      var movedIn = widths[toRegion]
      if (movedIn === undefined) delete widths[fromRegion]
      else widths[fromRegion] = movedIn
      if (movedOut === undefined) delete widths[toRegion]
      else widths[toRegion] = movedOut
    })
  }
  ...
  function applyBarConfig() {
    ...
    var islands = Util.isPlainObject(config.islands) ? config.islands : {}
    ...
    islandOutlineWidth = isFinite(outlineN) && outlineN >= 0 ? Math.round(outlineN) : 1
    var opacityN = Number(islands.outlineOpacity)
    islandOutlineOpacity = isFinite(opacityN) && opacityN > 0 ? Math.min(1, opacityN) : 0.25
    var widths = {}
    if (Util.isPlainObject(islands.widths)) {
      var regions = ["left", "center", "right"]
      for (var i = 0; i < regions.length; i++) {
        var n = Number(islands.widths[regions[i]])
        if (isFinite(n) && n > 0) widths[regions[i]] = Math.round(n)
      }
    }
    islandWidths = widths
    clearIslandWidthLive()
  ...
  component BarPanel: PanelWindow {
    ...
    // How far to shove the bar past its screen edge when hidden: bar thickness
    // plus, in island mode, the floating gap so no sliver peeks back.
    readonly property int parkDepth: root.barSize + (root.islandsActive ? root.islandEdgeGap : 0)

    margins {
      // Floating-island insets: the axis the bar hangs on lifts off by
      // islandEdgeGap, the perpendicular sides get islandSideGap.
      top: root.barHidden && root.position === "top" ? -barWindow.parkDepth
         : !root.islandsActive ? 0
         : root.vertical ? root.islandSideGap
         : (root.position === "top" ? root.islandEdgeGap : 0)
      bottom: root.barHidden && root.position === "bottom" ? -barWindow.parkDepth
         : !root.islandsActive ? 0
         : root.vertical ? root.islandSideGap
         : (root.position === "bottom" ? root.islandEdgeGap : 0)
      left: root.barHidden && root.position === "left" ? -barWindow.parkDepth
         : !root.islandsActive ? 0
         : root.vertical ? (root.position === "left" ? root.islandEdgeGap : 0)
         : root.islandSideGap
      right: root.barHidden && root.position === "right" ? -barWindow.parkDepth
         : !root.islandsActive ? 0
         : root.vertical ? (root.position === "right" ? root.islandEdgeGap : 0)
         : root.islandSideGap
    }
    ...
    Component {
      ...
      Item {
        ...
        readonly property var islandsById: ({
          left: leftIsland,
          center: centerIsland,
          right: rightIsland
        })
        ...
        IslandFrame {
          ...
          contentWidth: leftMods.implicitWidth
        ...
        IslandFrame {
          ...
          // Group-centered: the centerAnchor split is skipped in island mode,
          // Row implicit sizing is cycle-free and cannot deadlock modules.
          contentWidth: centerMods.implicitWidth
        ...
        IslandFrame {
          ...
          contentWidth: rightMods.implicitWidth
  ...
  component CustomCommandModule: WidgetButton {
    ...
    bar: root
    text: outputText !== "" ? root.sanitizeRichText(outputText) : String(setting("text", ""))
    ...
    Timer {
      interval: {
        var seconds = Number(customRoot.setting("interval", 5))
  ...
  component ModuleRow: Row {
    ...
    Repeater {
      model: rowRoot.entries

      ModuleSlot {
        required property var modelData
        entry: modelData
        region: rowRoot.section
      }
    }
  }

  // One floating pill per bar section. Sizing is content-driven plus a user
  // extra width; the whole pill can be grabbed on empty space to swap it with
  // another section, or resized from either end.
  component IslandFrame: Rectangle {
    id: frame

    required property string region
    property real contentWidth: 0
    // Config-derived, NOT measured: deriving visibility from contentWidth
    // creates a visible←width←visible cycle (slot widths read activeItem
    // visibility) that deadlocks every module to zero-size.
    ...
    // sections stack along the height (left modules at top, right at bottom,
    // matching the stock vertical layout), each pill centered on the edge.
    ...
    border.color: dropHot ? Color.accent : Qt.rgba(root.themeForeground.r, root.themeForeground.g, root.themeForeground.b, root.islandOutlineOpacity)
    opacity: dragged ? 0.88 : 1.0
    scale: dragged ? 1.02 : 1.0
    z: dragged ? 30 : dropHot ? 20 : 0
    ...
    transform: Translate {
      x: frame.dragged ? root.islandDragPos.x - root.islandDragStartPos.x : 0
      y: frame.dragged ? root.islandDragPos.y - root.islandDragStartPos.y : 0
    }

    // Grab-anywhere empty space: drag swaps this island with the one dropped
    // on; double-click keeps the stock transparency toggle. Declared before
    // the content so widget input wins everywhere they overlap.
    MouseArea {
      id: islandGesture
      ...
      anchors.fill: parent
      acceptedButtons: Qt.LeftButton
      cursorShape: frame.dragged ? Qt.ClosedHandCursor : Qt.ArrowCursor
      ...
      onPositionChanged: function(mouse) {
        ...
        var scenePoint = frame.mapToItem(null, mouse.x, mouse.y)
        if (!dragging) {
          var distance = Math.abs(mouse.x - pressedX) + Math.abs(mouse.y - pressedY)
          if (distance < dragThreshold) return
          dragging = true
          root.beginIslandMove(frame.region, Qt.point(scenePoint.x, scenePoint.y))
          if (!root.islandDragRegion) {
            dragging = false
            return
          }
        }

        root.updateIslandMove(Qt.point(scenePoint.x, scenePoint.y), barSurface.islandsById)
      }
